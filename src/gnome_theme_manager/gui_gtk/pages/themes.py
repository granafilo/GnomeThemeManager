"""Controller per la pagina 'Esplora temi' (Fase 5.2.1).

Questo modulo gestisce il caricamento dichiarativo della vista di esplorazione
e gestione dei temi installati. Espone il widget Adw.StatusPage per l'integrazione
nel Gtk.Stack dell'applicazione.
"""
...
class ThemesPage:
    """Controller della vista 'Esplora temi' per la GUI GTK4/Libadwaita."""
    ...
    def __init__(self, manager: "ThemeManager | None" = None) -> None:
        """Inizializza il controller caricando il template themes_page.ui.

        Args:
            manager: Istanza coordinatrice ThemeManager (non utilizzata in questa fase).

        Raises:
            FileNotFoundError: Se il template themes_page.ui non è presente nel filesystem.
        """
